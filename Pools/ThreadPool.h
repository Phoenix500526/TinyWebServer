...
private:
    ...
    void Add(F&& x) {
        ...
        m_notFull.wait(
            lck, [this] { return m_needStop || m_queue.size() < m_maxSize; });
...
public:
    ...
    void AddTask(const Task& task) { m_workqueue.put(task); }
    void AddTask(Task&& task) { m_workqueue.put(std::forward<Task>(task)); }

    size_t getTaskSize() { return m_workqueue.size(); }
