...
bool HttpRequest::Parse(Buffer& buff) {
  ...
  while (buff.ReadableBytes() && m_state != PARSE_STATE::FINISH) {
    const char* lineEnd =
        search(buff.Peek(), const_cast<const char*>(buff.BeginWrite()), CRLF,
               CRLF + 2);
...
void HttpRequest::ParsePost() {
  if (m_method == "POST" &&
    ...
    if (Path_Tag != DEFAULT_HTML_TAG.end()) {
      ...
      if (tag == 1) {
        if (Login(m_post["username"], m_post["password"]) ==
...
string HttpRequest::path() const { return m_path; }

string& HttpRequest::path() { return m_path; }

string HttpRequest::method() const { return m_method; }

string HttpRequest::version() const { return m_version; }
...
HttpRequest::LOGIN_STATUS HttpRequest::Login(const std::string& name,
                                             const std::string& pwd) {
  if (name.empty()) return LOGIN_STATUS::EMPTY_USERNAME;
  if (pwd.empty()) return LOGIN_STATUS::EMPTY_PASSWD;
  auto iter = USERTABLE.find(name);
  if (iter == USERTABLE.end()) {
    return LOGIN_STATUS::NO_SUCH_USER;
  }
  if (iter->second != pwd) return LOGIN_STATUS::WRONG_PASSWD;
...
bool HttpRequest::Register(const std::string& name, const std::string& pwd) {
  ...
  if (USERTABLE.find(name) != USERTABLE.end()) return false;
