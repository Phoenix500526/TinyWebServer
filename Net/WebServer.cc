...
WebServer::WebServer(const std::string& DBType, int port, int timeoutMS,
                     int sqlPort, const std::string& sqlUser,
                     const std::string& sqlPwd, const std::string& dbName,
                     int connPoolNum, int threadNum)
    : m_port(port),
      m_timeoutMS(timeoutMS),
      m_timer(new TimerHeap()),
  ...
  m_connPool->init(DBType, "localhost", sqlUser, sqlPwd, dbName, sqlPort,
                   connPoolNum);
...
void WebServer::AddClient(int fd, struct sockaddr_in addr) {
  ...
  if (m_timeoutMS > 0) {
    m_timer->AddTimer(fd, m_timeoutMS,
                      std::bind(&WebServer::CloseConn, this, &m_users[fd]));
  }
...
void WebServer::ExtentTime(HttpConn* client) {
  assert(client != nullptr);
  if (m_timeoutMS > 0) m_timer->AdjustTimer(client->GetFd(), m_timeoutMS);
}
...
bool WebServer::InitSocket() {
  ...
  ret = setsockopt(m_listenFd, SOL_SOCKET, SO_LINGER, &optLinger,
                   sizeof(optLinger));
  if (ret < 0) {
    close(m_listenFd);
   ...
   */
  ret = setsockopt(m_listenFd, SOL_SOCKET, SO_REUSEADDR, (const void*)&optval,
                   sizeof(int));
  if (ret == -1) {
