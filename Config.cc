...
Config::Config(const char* file, const string delim, const string comment)
    : m_Delimiter(delim), m_Comment(comment) {
    ifstream fin(file);
    if (!fin) throw File_Not_Found(string("Cannot find the file : ") + file);
    fin >> (*this);
}
...
ostream& operator<<(ostream& os, const Config& cf) {
    for (auto iter = cf.m_Content.cbegin(); iter != cf.m_Content.cend();
         ++iter) {
        os << iter->first << " " << cf.m_Delimiter << " " << iter->second
           << endl;
    }
    return os;
}
