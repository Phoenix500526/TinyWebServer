...
public:	
	Config(const char* file, std::string const delimiter, std::string const comment);
	...
	template <typename T> void Read(std::string const& key, T& value);
	friend std::ostream& operator<<(std::ostream& os, const Config& cf);
	friend std::istream& operator>>(std::istream& is, Config& cf);
private:
	static void Trim(std::string& str);
	template <typename T> T string_to_T(std::string const& value);
};
...
inline bool Config::string_to_T<bool>(std::string const& value){
	...
	for(auto iter = tmp.begin();iter != tmp.end();++iter)
		*iter = toupper(*iter);
	return !(tmp == std::string("NO") || tmp == std::string("FALSE") || tmp == std::string("0")
			|| tmp == std::string("NONE") || tmp == std::string("N") || tmp == std::string("F")
