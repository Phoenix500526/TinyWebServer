...
class HttpResponse_Derive : public HttpResponse {
 public:
  ...
  ~HttpResponse_Derive() {}
  char* GetFile() { return HttpResponse::File(); }
  size_t GetFileLen() const { return HttpResponse::FileLen(); }

  int GetCode() const { return HttpResponse::Code(); }

  std::string GetSrcDir() const { return HttpResponse::m_srcDir; }

  std::string GetPath() const { return HttpResponse::m_path; }

  bool GetIsKeepAlive() const { return HttpResponse::m_isKeepAlive; }

  void SetCode(int code) { HttpResponse::m_code = code; }
  void SetFile(char* file) { HttpResponse::m_mmFile = file; }
  void SetSrcDir(const std::string& new_srcDir) {
    HttpResponse::m_srcDir = new_srcDir;
  }
  void SetPath(const std::string& new_path) { HttpResponse::m_path = new_path; }
  void SetIsKeepAlive(bool iskeepAlive) {
    HttpResponse::m_isKeepAlive = iskeepAlive;
  }
  std::string GetFileType() { return HttpResponse::GetFileType(); }
  void ErrorHtml() { HttpResponse::ErrorHtml(); }
  void AddStateLine(Buffer& buff) { HttpResponse::AddStateLine(buff); }
  void AddHeader(Buffer& buff) { HttpResponse::AddHeader(buff); }
};
...
TEST(HttpResponseUnittest, ErrorContentTest) {
  ...
  std::string ErrorContent_BadRequest(
      "<html><title>Error</title><body bgcolor=\"ffffff\">500 : Bad "
      "Request\n<p>File Not "
      "Found</p><hr><em>TinyWebServer</em></body></html>");
  std::string result_1 = "Content-Length: " +
                         std::to_string(ErrorContent_BadRequest.size()) +
                         "\r\n\r\n" + ErrorContent_BadRequest;
  EXPECT_EQ(errorBuf.RetrieveAllAsString(), result_1);

  response.SetCode(403);
  response.ErrorContent(errorBuf, "Permission Denied");
  std::string ErrorContent_PermissionDenied(
      "<html><title>Error</title><body bgcolor=\"ffffff\">403 : "
      "Forbidden\n<p>Permission "
      "Denied</p><hr><em>TinyWebServer</em></body></html>");
  std::string result_2 = "Content-Length: " +
                         std::to_string(ErrorContent_PermissionDenied.size()) +
